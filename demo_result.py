...
class DEMO_Result():
    ...
    def LCC_operatingCost(self):
        return [a*self.LCCtable_.demoFactor.purchase_price/10000 for a in self.LCCtable_.project_loss_D]

    def LCC_maintenanceCost(self):
        return [a*self.LCCtable_.demoFactor.repairCost_Mtrans + c*self.LCCtable_.demoFactor.repairCost_Mline+e*self.LCCtable_.demoFactor.repairCost_Mcable+g*self.LCCtable_.demoFactor.repairCost_Munits+i*self.LCCtable_.demoFactor.repairCost_Lline+k*self.LCCtable_.demoFactor.repairCost_Lcable+m*self.LCCtable_.demoFactor.repairCost_Lunits \
    ...
    def LCC_faultCost(self):
        return [a*self.LCCtable_.demoFactor.faultCost_Mtrans + c*self.LCCtable_.demoFactor.faultCost_Mline+e*self.LCCtable_.demoFactor.faultCost_Mcable+g*self.LCCtable_.demoFactor.faultCost_Munits+i*self.LCCtable_.demoFactor.faultCost_Lline+k*self.LCCtable_.demoFactor.faultCost_Lcable+m*self.LCCtable_.demoFactor.faultCost_Lunits \
                for a,c,e,g,i,k,m in zip(np.float64(self.INPUT.trans10), np.float64(self.INPUT.medium_line)  \
                , np.float64(self.INPUT.medium_cable), np.float64(self.INPUT.medium_switchgear), np.float64(self.INPUT.low_line), np.float64(self.INPUT.low_cable), np.float64(self.INPUT.low_switchgear))]

    def LCC_removalCost(self):
        return [(a*self.LCCtable_.demoFactor.removalCost_Mtrans + c*self.LCCtable_.demoFactor.removalCost_Mline+e*self.LCCtable_.demoFactor.removalCost_Mcable+g*self.LCCtable_.demoFactor.removalCost_Munits+i*self.LCCtable_.demoFactor.removalCost_Lline+k*self.LCCtable_.demoFactor.removalCost_Lcable+m*self.LCCtable_.demoFactor.removalCost_Lunits-o*0.05)*self.LCCtable_.demoFactor.rely_discount_rate[29] \
    ...
    def LCC_increasePowerBenefits(self):
        return [a*self.LCCtable_.demoFactor.sales_profit/10000 for a in self.LCCtable_.project_IPsum_AJ]
    ...
    def LCC_increaseReliableBenefits(self):
        ...
        for a,b,c in zip(np.float64(self.LCCtable_.project_punish_AM),np.float64(self.LCCtable_.project_reduceLackPower_AU),np.float64(self.LCCtable_.project_lackPower_AT)):
            if self.INPUT.pro_name[i] == '110kV铜盂站10kV河陇线等线路自动化有效覆盖改造':
                print(a, b, c)
            i += 1
        return [(a*b+c*self.LCCtable_.demoFactor.overload_lackpower_factor)*self.LCCtable_.demoFactor.GDP_SUPPLY*self.LCCtable_.demoFactor.sum_rely_discount_rate/10000 for a,b,c in zip(np.float64(self.LCCtable_.project_punish_AM),np.float64(self.LCCtable_.project_reduceLackPower_AU),np.float64(self.LCCtable_.project_lackPower_AT))]

    # def LCC_preremoveBenefits():
    #     return [(0.023 - 0.0074) * self.LCCtable_.demoFactor.unqualified * a * fixed_parameters['I63' if self.input_parameters['N476'] == '低压配网网项目' else 'J63'] * fixed_parameters['R23'] * fixed_parameters['V23'] / 8760 / 10000
    def LCC_lowUCost(self):
        ...
        for a,b,c in zip(self.LCCtable_.project_avelowU_AX, np.float64(self.INPUT.lowV_trans), self.LCCtable_.project_userLowU_AW):
            if self.INPUT.pro_name[i] == '新增配变解决110kV东洋站10kV西新线10kV贵屿西美村4#公用台变电压偏低':
                print(a,b,c)
            i += 1
        return [a*b*c*self.LCCtable_.demoFactor.unqualified*self.LCCtable_.demoFactor.sum_rely_discount_rate*self.LCCtable_.demoFactor.GDP_SUPPLY/10000 for a,b,c in zip(self.LCCtable_.project_avelowU_AX, np.float64(self.INPUT.lowV_trans), self.LCCtable_.project_userLowU_AW)]
    ...
    def demo2(self):
        ...
        table = np.vstack((table, self.INPUT.pro_class))
